...
class DownloadCartObject(object):
    def __init__(
        self, projects=None, experiments=None, datasets=None, datafiles=None, id=None
    ):
...
def validate_objects(user, host, projects, experiments, datasets, datafiles):
    ...
    host_query = RemoteHost.objects.select_related("users").filter(
        pk=host, users__id=user.id
    )
    users_groups = [*user.groups.all().values_list("id", flat=True)]
    host_query |= RemoteHost.objects.select_related("groups").filter(
        pk=host, groups__id__in=users_groups
    )

    if not host_query.exists():
        raise ImmediateHttpResponse(
            HttpForbidden("User does not have access to remote host")
        )

    # Query for related projects, and unpack into list here for efficiency
    related_projects = [
        *{
            *RemoteHost.objects.filter(id=host)
    ...
    if projects:
        if Project.objects.filter(pk__in=projects).count() != len(projects):
            raise ImmediateHttpResponse(
                HttpNotFound("One or more Projects do not exist")
            )
        if Project.safe.all(user).filter(pk__in=projects).count() != len(projects):
            raise ImmediateHttpResponse(
                HttpForbidden("User does not have access to one or more Projects")
            )
    ...
    if experiments:
        if Experiment.objects.filter(pk__in=experiments).count() != len(experiments):
            raise ImmediateHttpResponse(
                HttpNotFound("One or more Experiments do not exist")
            )
        if Experiment.safe.all(user).filter(pk__in=experiments).count() != len(
            experiments
        ):
            raise ImmediateHttpResponse(
                HttpForbidden("User does not have access to one or more Experiments")
            )
        exp_projs = [
            *{
                *Experiment.objects.filter(pk__in=experiments)
                .select_related("project")
                .values_list("id", "project__id")
            }
        ]
        experiments = [
            id for (id, proj_id) in exp_projs if proj_id not in related_projects
        ]
    if datasets:
        if Dataset.objects.filter(pk__in=datasets).count() != len(datasets):
            raise ImmediateHttpResponse(
                HttpNotFound("One or more Datasets do not exist")
            )
        if Dataset.safe.all(user).filter(pk__in=datasets).count() != len(datasets):
            raise ImmediateHttpResponse(
                HttpForbidden("User does not have access to one or more Datasets")
            )
        set_projs = [
            *{
                *Dataset.objects.filter(pk__in=datasets)
                .prefetch_related("experiments", "experiments__project")
                .values_list("id", "experiments__project__id")
            }
        ]
        datasets = [
            id for (id, proj_id) in set_projs if proj_id not in related_projects
        ]
    if datafiles:
        if DataFile.objects.filter(pk__in=datafiles).count() != len(datafiles):
            raise ImmediateHttpResponse(
                HttpNotFound("One or more Datafiles do not exist")
            )
        if DataFile.safe.all(user).filter(pk__in=datafiles).count() != len(datafiles):
            raise ImmediateHttpResponse(
                HttpForbidden("User does not have access to one or more Datafiles")
            )
        file_projs = [
            *{
                *DataFile.objects.filter(pk__in=datafiles)
                .prefetch_related(
                    "dataset", "dataset__experiments", "dataset__experiments__project"
                )
                .values_list("id", "dataset__experiments__project__id")
            }
        ]
        datafiles = [
            id for (id, proj_id) in file_projs if proj_id not in related_projects
        ]
...
def validate_items_in_bundle(bundle):
    ...
    if not user.is_authenticated:
        raise ImmediateHttpResponse(
            HttpUnauthorized("User must be logged in to validate download cart")
        )
    ...
    if not any([proj_ids, exp_ids, set_ids, file_ids]):
        raise ImmediateHttpResponse(
            HttpBadRequest(
                "Please provide Projects/Experiments/Datasets/Datafiles for validation"
            )
        )
    # Query for related projects, and unpack into list here for efficiency
    proj_bad, exp_bad, set_bad, file_bad = validate_objects(
        user, host_id, proj_ids, exp_ids, set_ids, file_ids
    )
    invalid_items = {}
...
class ValidateAppResource(Resource):
    ...
    def create_return_invalid(self, bundle):
        ...
        bundle.obj = {
            "invalid_items": DownloadCartObject(
                projects=proj_bad,
                experiments=exp_bad,
                datasets=set_bad,
                datafiles=file_bad,
...
class TransferAppResource(Resource):
    ...
    def create_transfer(self, bundle):
        ...
        if not user.is_authenticated:
            raise ImmediateHttpResponse(
                HttpUnauthorized("User must be logged in to submit Globus transfer")
            )
        ...
        if any([proj_bad, exp_bad, set_bad, file_bad]):
            raise ImmediateHttpResponse(
                HttpForbidden(
                    "One or more objects could not be transferred to specified remote host"
                )
            )
        ...
        for proj in proj_ids:
            if not authz.has_download_access(bundle.request, proj, "project"):
                raise ImmediateHttpResponse(
                    HttpForbidden(
                        "You do not have download access for one or more projects"
                    )
                )
            query |= Project.objects.get(pk=proj).get_datafiles(user)
        for exp in exp_ids:
            if not authz.has_download_access(bundle.request, exp, "experiment"):
                raise ImmediateHttpResponse(
                    HttpForbidden(
                        "You do not have download access for one or more experiments"
                    )
                )
            query |= Experiment.objects.get(pk=exp).get_datafiles(user)
        for set in set_ids:
            if not authz.has_download_access(bundle.request, set, "dataset"):
                raise ImmediateHttpResponse(
                    HttpForbidden(
                        "You do not have download access for one or more datasets"
                    )
                )
        ...
        for file in file_ids:
            if not authz.has_download_access(bundle.request, file, "datafile"):
                raise ImmediateHttpResponse(
                    HttpForbidden(
                        "You do not have download access for one or more datafiles"
                    )
                )
        ...
        with transaction.atomic():
            newtransferlog = TransferLog.objects.create(
                initiated_by=user, remote_host=remote_vm, status=TransferLog.STATUS_NEW
            )
            newtransferlog.save()
            newtransferlog.datafiles.add(*downloadable_dfs)
            newtransferlog.save()

        bundle.obj = DownloadCartObject(
            projects=proj_bad,
            experiments=exp_bad,
            datasets=set_bad,
            datafiles=file_bad,
