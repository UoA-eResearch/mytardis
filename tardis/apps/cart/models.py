...
class RemoteHost(models.Model):
    ...
    endpoint = models.CharField(
        "Globus Endpoint", max_length=50, blank=False, null=False
    )
    managed = models.BooleanField(
        "Managed", default=True
    )  # managed by MyTardis service account
...
class TransferLog(models.Model):
    ...
    initiated_by = models.ForeignKey(
        User, related_name="initiated_by", on_delete=models.CASCADE
    )
    ...
    status = models.PositiveSmallIntegerField(
        choices=STATUS_CHOICES, null=False, default=STATUS_NEW
    )
