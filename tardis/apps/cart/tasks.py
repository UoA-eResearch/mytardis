...
@tardis_app.task(name="apps.cart.globus_transfers", ignore_result=True)
def globus_transfers(**kwargs):
    from .models import TransferLog

    transfers_to_submit = [
        *TransferLog.objects.filter(status=1).values_list("id", flat=True)
    ]
    transfers_to_check = [
        *TransferLog.objects.filter(status=2).values_list("id", flat=True)
    ]
    ...
    for transfer_id in transfers_to_check:
        if transfer_id:
            ...
            transfer_status(
                transfer_id
            )  # .apply_async(args=[copy.deepcopy(transfer_id)], **kwargs)
...
def transfer_status(transfer_id):
    ...
    from tardis.apps.cart.models import TransferLog
...
def transfer_submit(transfer_id):
    ...
    from tardis.apps.cart.models import TransferLog, RemoteHost
