...
hundred_words = ['absurd', 'act', 'addicted', 'advert', 'angry', 'apathetic',
                 'apparel', 'arrogant', 'barbarous', 'best', 'bite', 'bless',
                 'blink', 'blow', 'boundless', 'calculate', 'calculator',
                 'cannon', 'care', 'check', 'coil', 'compare', 'cool',
                 'dislike', 'doubt', 'drag', 'encourage', 'extra', 'feeble',
                 'fool', 'function', 'glue', 'hall', 'hellish', 'history',
                 'humdrum','impartial', 'jar', 'lame', 'leather', 'literate',
                 'little','lively', 'messy', 'mine', 'mother', 'muddled',
                 'mundane', 'naughty', 'neat', 'needless', 'nest', 'nose',
                 'nostalgic', 'oblong', 'perfect', 'perpetual', 'pest',
                 'planes','race','remarkable', 'replace', 'roomy', 'rule',
                 'run', 'safe','sail', 'secretive', 'shoes', 'splendid',
                 'staking', 'steep','steer', 'stiff', 'street', 'suggest',
                 'talented', 'talk','tangy', 'thank', 'thick', 'thought',
                 'tie', 'toes', 'tough','treatment', 'ubiquitous', 'ultra',
                 'uncle', 'unhealthy','unruly', 'unwritten', 'useless', 'vase',
...
def create_group_acl(group, object, group_flag):
    if group_flag == "adm":
        create_acl(group, django_group, object, download=True, write=True, sensitive=True)
    elif group_flag == "rd":
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        """
        parser.add_argument("--N", help="number of datafile objects to create. Choose 1 or 100",
                            type=int)
    ...
    def handle(self, *args, **options):
        ...
        for schema in [test_schema_proj, test_schema_exp, test_schema_set, test_schema_file]:
        ...
        sys.stderr.write("Creating managing group, institution, facility, and instrument...\n")
        ...
        test_facility = Facility(name="STRESSTEST_Facility",
                                     manager_group=test_manage_group,
                                     institution=test_institution)
        test_facility.save()
        test_instrument = Instrument(name="STRESSTEST_Instrument",
        ...
        for idx_proj in range(n_proj):
            ...
            for idx_exp in range(100):
                ...
                parset = ExperimentParameterSet(schema=test_schema_exp,
                                                experiment=testexp)
                parset.save()
                ...
                for idx_set in range(100):
                    ...
                    parset = DatasetParameterSet(schema=test_schema_set,
                                                 dataset=testset)
                    parset.save()
                    ...
                    for idx_file in range(100):
                        ...
                        parset = DatafileParameterSet(schema=test_schema_file,
                                                      datafile=testfile)
                        parset.save()
