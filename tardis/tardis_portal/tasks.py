...
def cleanup_dfos(**kwargs):
    ...
    dfos = DataFileObject.objects.filter(created_time__lte=wait_until, verified=False)
    for dfo in dfos:
...
def ingest_received_files(**kwargs):
    ...
    ingest_boxes = StorageBox.objects.filter(
        Q(attributes__key="type"), Q(attributes__value="receiving"), ~Q(master_box=None)
    )
...
def autocache(**kwargs):
    ...
    autocache_boxes = StorageBox.objects.filter(
        Q(attributes__key="autocache"), Q(attributes__value__iexact="True")
    )
...
@tardis_app.task(name="tardis_portal.datafile.save_metadata", ignore_result=True)
def df_save_metadata(df_id, name, schema, metadata):
    ...
    def get_schema(schema, name):
        ...
        try:
            ...
            new_schema = Schema(namespace=schema, name=name, type=Schema.DATAFILE)
            new_schema.save()
            return new_schema
    ...
    if not param_names:
        logger.warning("Bailing out of save_metadata because of 'not param_names'.")
    else:
        # Load datafile
        df = DataFile.objects.get(id=df_id)
        ...
        try:
            ps = DatafileParameterSet.objects.get(schema=data_schema, datafile=df)
            logger.warning("Parameter set already exists for {}".format(df.filename))
