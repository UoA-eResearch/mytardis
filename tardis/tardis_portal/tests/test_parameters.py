...
class ParametersTestCase(TestCase):
    ...
    def setUp(self):
        ...
        for data_type_str in [
                "NUMERIC", "STRING", "URL", "LINK", "FILENAME",
                "DATETIME", "LONGSTRING", "JSON"]:
            data_type = getattr(ParameterName, data_type_str)
            self.pnames[data_type] = ParameterName(
                schema=self.schema, name=data_type_str,
        ...
        self.pname_image_file = ParameterName(
            schema=self.schema, name="image file",
    ...
    def test_experiment_parameter_get(self):
        ...
        exp_numeric_param = ExperimentParameter.objects.create(
            ...
            name=self.pnames[ParameterName.NUMERIC], numerical_value=123.456)
        ...
        exp_string_param = ExperimentParameter.objects.create(
            parameterset=exp_parameterset,
            name=self.pnames[ParameterName.STRING], string_value="value1")
        ...
        self.assertEqual(
            str(exp_string_param), "Experiment Param: STRING=value1")

        # URL
        url = "http://example.com/"
        exp_url_param = ExperimentParameter.objects.create(
            parameterset=exp_parameterset,
            name=self.pnames[ParameterName.URL], string_value=url)
        self.assertEqual(
            exp_url_param.get(), "<a href='%s'>%s</a>" % (url, url))
        ...
        exp_filename_param = ExperimentParameter.objects.create(
            parameterset=exp_parameterset,
            name=self.pnames[ParameterName.FILENAME], string_value="image.png")
        ...
        exp_json_param = ExperimentParameter.objects.create(
            parameterset=exp_parameterset,
            name=self.pnames[ParameterName.JSON], string_value="[1, 2, 3]")
    ...
    def test_image_filename_parameters(self):
        ...
        exp_image_param = ExperimentParameter.objects.create(
            parameterset=exp_parameterset,
            name=self.pname_image_file, string_value="image.png")
        expected_image_url = (
    ...
    def test_link_urls(self):
        ...
        self.assertEqual(
            datafile_link_param.get(), "<a href='%s'>%s</a>" % (dataset_url, dataset_url))
