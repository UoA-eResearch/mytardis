...
def remove_user_from_group(request, group_id, username):
    ...
    if user.groups.filter(name=group.name).count() == 0:
        return HttpResponse(
            "User %s is not member of that group." % username, status=400
        )
...
def add_experiment_access_user(request, experiment_id, username):
    ...
    try:
        ...
        if not user.is_active:
            return HttpResponse("User %s is inactive." % (username), status=400)
    except User.DoesNotExist:
        return HttpResponse("User %s does not exist." % (username), status=400)
...
def add_experiment_access_group(request, experiment_id, groupname):
    ...
    try:
        ...
        return HttpResponse("Group %s does not exist" % (groupname), status=400)
    ...
    if acl.count() > 0:
        ...
        return HttpResponse(
            "Could not add group %s " "(It has already been added)" % (groupname),
            status=400,
        )
